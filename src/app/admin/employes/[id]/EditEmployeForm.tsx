  ...
  if (success) {
        return (
                <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center">
                        <p className="text-green-800 font-medium">Modifications enregistrees !</p>p>
                        <p className="text-green-600 text-sm mt-1">Redirection en cours...</p>p>
                </div>div>
              )
  }
    ...
    return (
          <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
            {error && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">{error}</div>div>
                )}
                <div className="grid grid-cols-2 gap-4">
                        <div>
                                  <label className={labelCls}>Prenom *</label>label>
                                  <input className={inputCls} value={form.prenom} onChange={e => set('prenom', e.target.value)} required />
                        </div>div>
                        <div>
                                  <label className={labelCls}>Nom *</label>label>
                ...
                <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">L&apos;email ne peut pas etre modifie</p>p>
                </div>div>
                <div>
                        <label className={labelCls}>Poste / Titre</label>label>
                        <input className={inputCls} value={form.poste} onChange={e => set('poste', e.target.value)} placeholder="Ex: Infirmier(e), Technicien(ne)..." />
                </div>div>
                <div>
                        <label className={labelCls}>Departement</label>label>
                        <select className={inputCls} value={form.departement_id} onChange={e => set('departement_id', e.target.value)}>
                                  <option value="">-- Aucun departement --</option>option>
                          {departements.map(d => (
                        <option key={d.id} value={d.id}>{d.nom}</option>option>
                      ))}
                        </select>select>
                </div>div>
                <div>
                        ...
                        <select className={inputCls} value={form.role} onChange={e => set('role', e.target.value)}>
                                  <option value="employe">Employe</option>option>
                                  <option value="gestionnaire">Gestionnaire</option>option>
                                  <option value="admin">Administrateur</option>option>
                        </select>select>
                </div>div>
                <div className="flex items-center gap-3">
                        <input type="checkbox" id="actif" checked={form.actif} onChange={e => set('actif', e.target.checked)}
                                    className="w-4 h-4 text-blue-600 rounded border-gray-300" />
                        <label htmlFor="actif" className="text-sm font-medium text-gray-700">Employe actif</label>label>
                </div>div>
